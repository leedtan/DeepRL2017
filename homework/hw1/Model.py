...
class Model():
    ...
    def __init__(self,  size_obs, size_act, net_struct = [200, 200, 200, 200, 200], name='dbg', adv_mag = ''):
        name = name + adv_mag
        ...
        if 1:
            for idx, l in enumerate(net_struct):
                act_trn, act_tst = ops.cascade_bn_relu_trn_tst(
                        act_trn, prev_layer_size, l, name=name+'layer' + str(idx), input_tst = act_tst)
                prev_layer_size += l
                
            w = tf.Variable(tf.random_uniform([prev_layer_size, size_act],minval = -1., maxval = 1.), name=name+'net_output_w') * 1e-3
            b = tf.Variable(tf.random_uniform([size_act],minval = -1., maxval = 1.), name=name+'net_output_bias') * 1e-3
        else:
            for idx, l in enumerate(net_struct):
                act_trn = ops.linear(act_trn, l, 'layer' + str(idx))
            w = tf.Variable(tf.random_uniform([l, size_act],minval = -1., maxval = 1.), name=name+'net_output_w') * 1e-2
            b = tf.Variable(tf.random_uniform([size_act],minval = -1., maxval = 1.), name=name+'net_output_bias') * 1e-2
